...
The sd is *NA* for carrier **OO** because there is only one flight for that carrier. Since standard deviation is calculated using $n -1$ in the denominator, when $n=1$ the denominator becomes 0, making the sd undefined. For carriers **AS, F9,** and **HA**, the sd is 0 because all flights for these carriers have the same distance, there's no variation in the data. The code below, which uses a sample of flight data, demonstrates this:

```{r}
#create a sample flight dataset 
carrier3<- c("OO","AS","AS","HA","HA","HA")
distance3 <-c(733, 2402,2402, 4983, 4983, 4983)# oo should have sd Na since it's only one and the HA and AS should have 0 since the distances for each carrier are the same.
...
```{r}
routes3<-routes2|> left_join(airlines,join_by(carrier))
routes3
```
...
```{r}
best_worst<-routes3|> group_by(origin, dest)|> summarise(best_airline=name[which.min(ave_delay)], worst_airline = name[which.max(ave_delay)], .groups = "drop")
best_worst
```
...
```{r}
airline_delays <- routes3 |> 
  group_by(origin, dest) |> 
  summarise(
    best_airline = name[which.min(ave_delay)],
    min_delay = min(ave_delay, na.rm = TRUE),
    worst_airline = name[which.max(ave_delay)],
    max_delay = max(ave_delay, na.rm = TRUE),
    .groups = "drop"
  )
airline_delays
diff_routes <- airline_delays |> 
  mutate(airline_difference = abs(max_delay - min_delay)) |> 
  arrange(desc(airline_difference))
diff_routes
max_diff <- diff_routes |> slice(1)
max_diff

```

JFK - ATL route has the greatest difference between the best and worst performing airlines.
...
flights |> 
  ...
  summarise(num_flights = n()) #checking the number of flights for EV on the route 
```

ExpressJet Airlines Inc has only one flight on this route, therefore the delay might be because of high turnaround times ( unloading, refueling, maintenance, loading for the next flight etc)
...
The glucose and height column have Na values. There's a typo in the disease status column ( "healthy"; "Healthy)
