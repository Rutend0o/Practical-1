...
```{r}
dist_tbl1<- flights|>filter(month ==1)|>distinct(carrier)|>group_by(carrier)
ungroup() 
dist_tbl1|>summarise(dist_mean = mean(distance, na.rm=TRUE), dist_sd= sd(distance, na.rm=TRUE),.groups="drop"
)|>arrange(dist_mean)
```
...
flights |> 
  group_by(origin, dest) |>              
  summarise(airlinesflying = n_distinct(carrier)) |> filter(airlinesflying > 1)|>  
 group_by(origin, dest,carrier)|>summarise(ave_delay=mean(arr_delay, na.rm=TRUE))
```
